...
class GroundSpeeds():
    ...
    def get_euler_angles(self):
        if 1 - self.t_hb[0, 2] > 2e-4:
            phi = np.arctan(self.t_hb[1, 2] / self.t_hb[2, 2])
            theta = np.arcsin(-self.t_hb[0, 2])
            sai = np.arctan(-self.t_hb[0, 1], self.t_hb[0, 0])
        else:
            phi = 0
            theta = np.arcsin(-self.t_hb[0, 2])
            sai = np.arctan(-self.t_hb[1, 0] / self.t_hb[1, 1])
